...
const updateTokenRegistrationRestrictController = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const tokenRegistrationToUpdateExists =
      await findOneByIdTokenRegistrationService(
        tokenRegistrationId,
        req.user.id
      );
...
export default updateTokenRegistrationRestrictController;
