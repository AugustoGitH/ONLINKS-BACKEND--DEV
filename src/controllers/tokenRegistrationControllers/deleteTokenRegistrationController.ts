...
const deleteTokenRegistrationController = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const tokenRegistrationDeleted = await deleteTokenRegistrationService(id);
...
export default deleteTokenRegistrationController;
