...
const createShortenerLinkService = async (
  createShortenerLink: CreateShortenerLink
): Promise<ShortenerLink> => {
  try {
    const { data } = await api.shortenerOnlinks.post<ShortenerLink>(
      shortenerOnlinksRoutes.CREATE,
      createShortenerLink
    );
...
export default createShortenerLinkService;
